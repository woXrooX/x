		...
		return `
			<container class="align-items-center padding-5">
				<column class="justify-content-center align-items-center surface-${type} padding-8 gap-2">
					<p class="display-flex flex-row align-items-center gap-0-2 text-color-${type} text-size-2rem">
						<x-svg name="type_${type}" class="text-size-2-5rem" color="var(--color-${type})"></x-svg>
						${content_title}
					</p>
					<p class="text-size-1-1rem text-align-center">${content}</p>
				</column>
			</container>
		`;
	}
