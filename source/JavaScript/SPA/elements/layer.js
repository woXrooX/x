export default class Layer extends HTMLElement {
    ...
    static #on_show_update_layers(){
        ...
        if (layers.length >= 4) layers[layers.length - 4].classList.add("cover");

        // Manage layer visibility on click with a slight delay
    ...
    static #render_layers(){
        ...
        for (const index in visible_layers) {
            ...
            if (index === visible_layers.length - 1) visible_layers[index].style.bottom = "10px";
        ...
        // Remove the transition class from all layers that don't need it anymore
        // This happens after all layers are properly positioned
        layers.forEach(layer => {
            if (!layer.classList.contains("cover")) return;
            
            // Only remove the class from layers that are not among the last 3
            if (!visible_layers.includes(layer)) {
                // After transition completes, remove the class
                setTimeout(() => {
                    layer.classList.remove("cover");
