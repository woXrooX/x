			...
			email_html = f"""
				...
				<html lang="en">
					<head>
						...
						<title>{Globals.PROJECT_LANG_DICT.get(Globals.CONF["project_name"], {}).get(Globals.CONF["default"]["language"]["fallback"], "x")}</title>
					</head>
					<body style="height: 100% !important; width: 100% !important; padding: 10px; margin: 0;">
						<header style="text-aligment:center; color: white; padding:5px; width:100%;">
							{Globals.PROJECT_LANG_DICT.get(Globals.CONF["project_name"], {}).get(Globals.CONF["default"]["language"]["fallback"], "x")}
						</header>
						<main>{content}</main>
						<footer></footer>
					</body>
