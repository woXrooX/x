if __name__ != "__main__":
	...
	class Payment:
		...
		def create_intent(
			...
			if Stripe.initialized is not True: return Response.make(type="error", message="Stripe_is_not_initialized", HTTP_response_status_code=401)

			try:
				customer_id = Customer.create_customer()
				if customer_id is False:
					Log.error(f"Payment.create_intent(): could_not_create_customer")
					return Response.make(type="error", message=str(e), HTTP_response_status_code=500)
				...
				return Response.make(type="success", message="success", data=payment_intent.client_secret)

			except stripe.error.StripeError as e:
				Log.error(f"Payment.create_intent(): StripeError -> {e}")
				return Response.make(type="error", message=str(e), HTTP_response_status_code=400)
		...
		def create_subscription(subscription_name):
			...
			if subscription_name not in Product.subscription_products: return Response.make(type="error", message="invalid_request", HTTP_response_status_code=400)
			...
			try:
				...
				return Response.make(type="success", message="success", data=response_data)
