...
def files_users(ID, PRIVATE_PUBLIC, PATH_AND_FILE):
	if PRIVATE_PUBLIC == "private":
		if "user" in session and session["user"]["id"] == ID:
			...
			try: return send_from_directory(f"{Globals.X_PATH}/project_files/users/{ID}/private/", path=PATH_AND_FILE, as_attachment=False)
			...
			try: return send_from_directory(f"{Globals.X_PATH}/project_files/users/{ID}/public/", path=PATH_AND_FILE, as_attachment=False)
			except: return abort(404)
